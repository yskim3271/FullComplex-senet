...
class GhostModule(nn.Module):
    def __init__(self, in_channel, out_channel, ratio=2, kernel_size=1, dw_size=3):
        ...
        self.dw_kernel_size = dw_size

        self.primary_conv = nn.Sequential(
            nn.Conv2d(in_channel, self.init_channel, kernel_size=kernel_size, stride=1, padding=get_padding(kernel_size), bias=False),
            nn.InstanceNorm2d(self.init_channel, affine=True),
            nn.PReLU(self.init_channel)
        )
        self.cheap_conv = nn.Sequential(
            nn.Conv2d(self.init_channel, self.cheap_channel, kernel_size=dw_size, stride=1, padding=1, groups=self.init_channel, bias=False),
            nn.InstanceNorm2d(self.cheap_channel, affine=True),
            nn.PReLU(self.cheap_channel)
        )
        self.short_conv = nn.Sequential(
            nn.Conv2d(in_channel, out_channel, kernel_size=kernel_size, stride=1, padding=get_padding(kernel_size), bias=False),
    ...
    def forward(self, x):
        ...
        return out[:,:self.out_channel,:,:]*F.interpolate(self.gate_fn(res), size=(out.shape[-2], out.shape[-1]), mode='nearest')
...
class GhostTFEncoder(nn.Module):
    def __init__(self, in_channel, out_channel, ratio=2, kernel_size=1, dw_size=3):
        ...
        self.ghost = GhostModule(in_channel, out_channel, ratio=ratio, kernel_size=kernel_size, dw_size=dw_size)
...
class GhostTFBlock(nn.Module):
    def __init__(self, ratio=2, kernel_size=1, dw_size=3):
        super(GhostTFBlock, self).__init__()
        
        self.encoder1 = GhostTFEncoder(64, 96, ratio=ratio, kernel_size=kernel_size, dw_size=dw_size)
        self.encoder2 = GhostTFEncoder(96, 128, ratio=ratio, kernel_size=kernel_size, dw_size=dw_size)
...
class PrimeKnetv5(nn.Module):
    def __init__(self, 
                 fft_len=400,
                 dense_channel=64,
                 sigmoid_beta=2.0,
                 ):
        super(PrimeKnetv5, self).__init__()
        self.fft_len = fft_len
        self.dense_channel = dense_channel
        self.dense_encoder = DenseEncoder(dense_channel, in_channel=2)
        self.ghost_tfblock = GhostTFBlock(ratio=2, kernel_size=3, dw_size=3)
