...
class ComplexLeakyModReLU2d(nn.Module):
    ...
    def __init__(self, C, negative_slope=0.2, bias=True, eps=1e-8):
        ...
        self.eps = eps
        self.b = nn.Parameter(torch.zeros(1, C, 1, 1)) if bias else None
    def forward(self, xr, xi):
        ...
        for i in range(depth):
            ...
            self.layers.append(
                nn.ModuleDict({
                    ...
                    "act": ComplexLeakyModReLU2d(dense_channel, negative_slope=0.2, bias=True)
                })
            )
...
class ComplexDenseEncoder(nn.Module):
    def __init__(self, 
        ...
        self.pre_act = ComplexLeakyModReLU2d(dense_channel, negative_slope=0.2, bias=True)
        ...
        self.post_act = ComplexLeakyModReLU2d(dense_channel, negative_slope=0.2, bias=True)
...
class ComplexSENet(nn.Module):
    ...
    def forward(self, noisy_com): # [B, F, T]
        ...
        # Apply mask (broadcast to mag shape)
        # restore shape [B, T, F]
