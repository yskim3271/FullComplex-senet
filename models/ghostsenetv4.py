...
class Ghost_DDB(nn.Module):
    def __init__(self, dense_channel, kernel_size=(3, 3), depth=4, ratio=2, attention=False):
        ...
        if attention:
            self.attn_block = nn.Sequential(
                nn.Conv2d(dense_channel, dense_channel, kernel_size=kernel_size, padding=get_padding_2d(kernel_size), bias=False),
        ...
        for i in range(depth):
            ...
            dense_conv = nn.Sequential(
                nn.Conv2d(self.dense_channel*(i+1), self.init_channel, kernel_size, dilation=(dil, 1),
                          padding=get_padding_2d(kernel_size, dilation=(dil, 1)), bias=True),
    ...
    def forward(self, x):
        skip = x
        if self.attention:
            res = self.attn_block(F.avg_pool2d(x, kernel_size=2, stride=2))
        ...
        if self.attention:
            return x[:,:self.dense_channel,:,:]*F.interpolate(self.gate_fn(res), size=(x.shape[-2], x.shape[-1]), mode='nearest')
        else:
            return x
...
class DenseEncoder(nn.Module):
    def __init__(self, dense_channel, ratio=2, in_channel=2):
        ...
        self.dense_block = Ghost_DDB(dense_channel, depth=4, ratio=ratio, attention=False) # [b, h.dense_channel, ndim_time, h.n_fft//2+1]
...
class MaskDecoder(nn.Module):
    def __init__(self, 
        ...
        self.n_fft = n_fft
        self.dense_block = Ghost_DDB(dense_channel, kernel_size=(3, 3), depth=4, ratio=2, attention=True)
...
class PhaseDecoder(nn.Module):
    def __init__(self, 
        ...
        self.dense_block = Ghost_DDB(dense_channel, kernel_size=(3, 3), depth=4, ratio=ratio, attention=True)
...
class GhostSEnetV4(nn.Module):
    def __init__(self, 
        ...
        super(GhostSEnetV4, self).__init__()
